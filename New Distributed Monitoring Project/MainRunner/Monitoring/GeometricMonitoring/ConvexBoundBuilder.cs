...
namespace Monitoring.GeometricMonitoring
{
    ...
    {
        private Func<Vector<double>, double> Compute { get; }
        private Predicate<double> IsInBound { get; }
        private Dictionary<norm, ClosestPointFromPoint> GetClosestPointOfNorm { get; }

        private ConvexBoundBuilder(Func<Vector<double>, double> compute, Predicate<double> isInBound, Dictionary<int, ClosestPointFromPoint> getClosestPointOfNorm)
        {
            Compute = compute;
            IsInBound = isInBound;
        ...
        public static ConvexBoundBuilder Create(Func<Vector<double>, double> computeFunction, Predicate<double> isInBound) 
